...
def simple_model():
    model = nn.Sequential(
        ...
        nn.Dropout(p=0.5),
        nn.Conv1d(in_channels=128, out_channels=128, kernel_size=8, stride=1, padding=3 , bias=False), #หนูเพิ่ม padding เข้าไปเพราะว่า kernel size(8) ใหญ่กว่า input size(4) ค่ะ
        nn.Conv1d(in_channels=128, out_channels=128, kernel_size=8, stride=1, padding=3 ,bias=False), #และที่เพิ่ม padding เป็นขนาด 3 เพราะว่า (kernel size -1)/2 = 3.5 ค่ะ
